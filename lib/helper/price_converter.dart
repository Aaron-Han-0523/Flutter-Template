...
class PriceConverter {
  static String convertPrice(BuildContext context, double price,
    ...
    return '${inRight ? '' : Provider.of<SplashProvider>(context, listen: false).myCurrency?.symbol}'
        '${(singleCurrency ? price : price * Provider.of<SplashProvider>(context, listen: false).myCurrency!.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency!.exchangeRate)).toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
        '${inRight ? '' : ''}';
  }
